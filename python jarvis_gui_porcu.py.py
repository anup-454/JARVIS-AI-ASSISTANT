...
WEATHER_API_KEY = "YOUR_OPENWEATHERMAP_API_KEY_HERE"  # <-- Put your API key here
WEATHER_UNITS = "metric"        # metric or imperial

# Deepseek API Key - Get from https://platform.deepseek.com/
# Get your API key: https://platform.deepseek.com/api-keys
DEEPSEEK_API_KEY = "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"  # Replace with your actual key starting with 'sk-'
...
# Optimize for low pitch sound and accurate command capture
recognizer.energy_threshold = 1000  # Ultra-low threshold for low-pitch sounds
recognizer.dynamic_energy_threshold = True  # Adapt to ambient noise in real-time
recognizer.pause_threshold = 0.7  # Longer pause to ensure full phrase capture
recognizer.non_speaking_duration = 0.2  # Shorter initial silence period for faster detection
recognizer.phrase_time_limit = 15  # Extended to 15 seconds for complete commands

# Sounddevice config for audio capture
SAMPLE_RATE = 16000
CHUNK_DURATION = 3  # Increased to 3 seconds for more complete phrase capture
    ...
    with tts_lock:
        try:
            ...
            if USE_PYTTSX3:
                ...
                # Detect and adjust for voice characteristics
                voices = engine.getProperty('voices')
                if len(voices) > 0:
                    # Get the first available voice (system default)
                    default_voice = voices[0]
                    engine.setProperty('voice', default_voice.id)
                    
                    # Determine if voice is male or female based on voice name
                    voice_name = default_voice.name.lower()
                    is_female_voice = any(name in voice_name for name in ['woman', 'female', 'female voice', 'female voice 1', 'female voice 2'])
                    is_male_voice = any(name in voice_name for name in ['male', 'man', 'male voice', 'male voice 1'])
                    
                    # Adjust rate: female voices and high-pitched voices speak slower naturally
                    # so increase rate for them; male/low voices need slight increase too
                    if is_female_voice:
                        adjusted_rate = 200  # Faster for female voices
                    elif is_male_voice:
                        adjusted_rate = 180  # Slightly faster for male voices
                    else:
                        adjusted_rate = 190  # Default faster rate
                    
                    engine.setProperty('rate', adjusted_rate)
                    print(f"[TTS] Voice: {default_voice.name}, Adjusted rate: {adjusted_rate}")
                else:
                    engine.setProperty('rate', 180)
                    print(f"[TTS] No voice properties available, using default rate: 180")
...
def get_health_tips():
    """Get random health tips from a built-in collection."""
    health_tips = [
...
def get_news():
    ...
    try:
        ...
        fallback_news = "I couldn't fetch live news at this moment. Please check a news website directly."
...
def search_youtube(query):
    ...
    try:
        ...
        # Try to use yt-dlp to find the first video
        try:
            import yt_dlp
            ydl_opts = {
                'quiet': True,
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if result and 'entries' in result and len(result['entries']) > 0:
                    ...
                    print(f"[YouTube] Opening first video: {video_url}")
                    webbrowser.open(video_url)
                    return True, f"Playing first video for {query}."
        except ImportError:
            print(f"[YouTube] yt-dlp not available, falling back to search results")
        except Exception as e:
            print(f"[YouTube] yt-dlp search failed: {e}, falling back to search results")
...
def search_google(query):
    """Search for a query on Google and open the first result."""
    try:
        from urllib.parse import quote
        
        print(f"[Google] Searching for: {query}")
        
        # Try to use googlesearch library to find the first result
        try:
            from googlesearch import search
            results = search(query, num_results=1, sleep_interval=0.5)
            first_result = next(results, None)
            if first_result:
                print(f"[Google] Opening first result: {first_result}")
                webbrowser.open(first_result)
                return True, f"Opening first result for {query}."
        except ImportError:
            print(f"[Google] googlesearch not available, falling back to search results")
        except Exception as e:
            print(f"[Google] googlesearch failed: {e}, falling back to search results")
        
        # Fallback: Open Google search results
        encoded_query = quote(query)
        search_url = f"https://www.google.com/search?q={encoded_query}"
        print(f"[Google] Opening search results: {search_url}")
        webbrowser.open(search_url)
        return True, f"Searching Google for {query}."
    except Exception as e:
        return False, f"Failed to search Google: {e}"
...
def handle_command(command_text):
    ...
    if "youtube" in text and any(w in text for w in ["search", "play", "find", "look for"]):
        # Extract the search query
        # Patterns: "search youtube for [query]", "play [query] on youtube", "find [query] youtube"
        search_query = None
    ...
    # Google/Chrome search command
    if any(w in text for w in ["google", "search", "search google", "search for"]) and "youtube" not in text and "spotify" not in text:
        # Extract the search query
        search_query = None
        
        # Pattern 1: "search for X" or "search google for X"
        if "search" in text and "for" in text:
            search_query = text.split("search", 1)[1].split("for", 1)[1].strip()
        # Pattern 2: "google X"
        elif "google" in text:
            parts = text.split("google")
            if len(parts) > 1 and parts[1].strip():
                search_query = parts[1].strip()
        # Pattern 3: "open chrome and search for X"
        elif "open chrome" in text and "search" in text:
            search_query = text.split("search", 1)[1].split("for", 1)[1].strip() if "for" in text else text.split("search", 1)[1].strip()
        
        if search_query:
            ok, msg = search_google(search_query)
            speak(msg)
            return
        else:
    ...
    if any(w in text for w in ["open", "launch", "start"]):
        ...
        # Special handling for YouTube - if just "open youtube"
        if tgt.lower() == "youtube":
            print(f"Opening website: {tgt}")
            ok, msg = open_website(tgt)
        # Check if it's a website (has dots or known website keywords)
        elif '.' in tgt or any(w in tgt for w in ['google', 'youtube', 'facebook', 'twitter', 'github', 'instagram']):
            print(f"Opening website: {tgt}")
            ok, msg = open_website(tgt)
        else:
            # Try to open as app
...
def enhance_low_frequency_audio(audio_data):
    ...
    try:
        ...
        # Boost low frequencies by enhancing the signal envelope
        enhanced = audio_float.copy()
        for i in range(1, len(enhanced)):
            # Smooth out rapid changes to emphasis slower (lower freq) variations
...
def detect_low_frequency_amplitude(audio_data):
    ...
    try:
        # For int16 audio, check RMS (Root Mean Square) which is better for low frequencies
        audio_float = audio_data.astype(float)
...
def recognize_with_fallback(audio, max_retries=2):
    """Try Google recognizer with retries, fall back to PocketSphinx if available.
    ...
    for attempt in range(max_retries):
        try:
            ...
            # network/DNS error — retry a couple times briefly
            if attempt < max_retries - 1:
                time.sleep(0.5)
                continue
            # final attempt: try offline PocketSphinx if installed
            try:
                print("[Recognition] Falling back to PocketSphinx (offline)")
...
def listen_for_wakeword_and_command():
    """Listen for commands with enhanced sensitivity for low voices and low-pitch sounds."""
    global should_stop
    ...
    try:
        ...
        duration = 3
        print(f"[Listening] Recording for {duration} seconds...")
        ...
        print(f"[Audio] Max: {max_amplitude}, RMS: {rms_amplitude}")
        ...
        if max_amplitude < 8000:
            ...
            print(f"[Audio] Amplified audio (original max: {max_amplitude}, RMS: {rms_amplitude})")
        
        audio = sr.AudioData(audio_data.tobytes(), SAMPLE_RATE, 2)
    except Exception as e:
        # Silently skip recording errors - continue listening
        print(f"[Recording] Error: {e}")
        return
    
    try:
        print("[Recognition] Processing audio...")
        text = recognize_with_fallback(audio)
        if not text:
            # Could not recognize (network or offline failure)
            return
        ...
        # Silently skip - user didn't speak or audio was unclear
        return  # Continue listening
    except sr.RequestError:
        # Silently skip - network or API error, keep listening
        return  # Continue listening
    except Exception:
        # Silently skip any other errors - continue listening
        return  # Continue listening
...
def handle_command_gui(command_text, gui):
    ...
    # Google/Chrome search command
    elif any(w in text for w in ["google", "search", "search google", "search for"]) and "youtube" not in text and "spotify" not in text:
        search_query = None
        if "search" in text and "for" in text:
            search_query = text.split("search", 1)[1].split("for", 1)[1].strip()
        elif "google" in text:
        ...
        if search_query:
            ok, response = search_google(search_query)
        else:
...
def listen_for_wakeword_and_command_gui(gui):
    """Listen for commands with enhanced sensitivity for low-pitch sounds - GUI version"""
    global should_stop
    if not microphone_available:
        return
